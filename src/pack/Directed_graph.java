...
public class Directed_graph {//图的功能
  ...
  public static void CreateGraph(String path) throws IOException { 
    int flag,point;
  ...
  public static void showDirectedGraph() {//展示图的函数主体
    ...
    String Order_r;
    int i,j;
    GraphViz gViz = new GraphViz("D:\\www\\Lab4\\Lab4\\src","D:\\bin\\dot.exe","miao");
    gViz.start_graph();
  ...
  public static String[] Getbridegewords(String word1, String word2) {//获取桥接词的关键函数
	int[]words_temp_pos;
	int i,j,temp_num = 0;
  ...
  public static String queryBridgeWords(String word1, String word2){//获取桥接词的函数主体 
    String temp[] , stranswer , strresult;
    
    StringBuffer result;
    int flag1 = 0 , flag2 = 0 ,answer , len , i;
    for (i = 0;i < G.point_num;i++) {
    ...
    temp = Getbridegewords(word1,word2);
    for (len = 0;;len++) {
      if (temp[len]==null) {
        break;
      }
	...
	public static String generateNewText(String word1) {//生成新文本的函数
    	int word_num , i , random_position , real_string_array_length;
		...
		else
		{
			for(i=0;i<word_num-1;i++)
			{
				...
				if(G.Search(words[i])!=-1)
				{
					...
					{
						bridge_words=Getbridegewords(words[i],words[i+1]);
	...
	public static String calcShortestPath(String word1, String word2)//计算最短路径的函数
    {
    	...
    	int int_word1 = -1 , int_word2 = -1 , answer;
    	StringBuffer result;
    	String stranswer,strresult;
